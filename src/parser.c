...
static rbs_location_t *rbs_location_current_token(parserstate *state) {
  return alloc_location(&state->allocator, state->current_token.range);
}
...
static bool parse_type_name(parserstate *state, TypeNameKind kind, range *rg, rbs_typename_t **typename) {
  ...
  while (
    ...
    rbs_location_t *symbolLoc = alloc_location(&state->allocator, state->next_token.range);
  ...
  rbs_location_t *loc = alloc_location(&state->allocator, namespace_range);
  ...
  success: {
    ...
    rbs_location_t *symbolLoc = alloc_location(&state->allocator, state->current_token.range);
    ...
    *typename = rbs_typename_new(&state->allocator, alloc_location(&state->allocator, *rg), namespace, symbol);
    return true;
  }
...
static bool parse_function_param(parserstate *state, rbs_types_function_param_t **function_param) {
  ...
  if (state->next_token.type == pCOMMA || state->next_token.type == pRPAREN) {
    ...
    rbs_location_t *loc = alloc_location(&state->allocator, param_range);
...
static bool parse_keyword(parserstate *state, rbs_hash_t *keywords, rbs_hash_t *memo) {
  ...
  if (rbs_hash_find(memo, (rbs_node_t *) key)) {
    ...
    rbs_location_t *loc = alloc_location(&state->allocator, state->current_token.range);
...
static bool parse_optional(parserstate *state, rbs_node_t **optional) {
  ...
  if (state->next_token.type == pQUESTION) {
    parser_advance(state);
    rg.end = state->current_token.range.end;
    rbs_location_t *location = alloc_location(&state->allocator, rg);
...
static bool parse_function(parserstate *state, bool accept_type_binding, parse_function_result **result) {
  ...
  if (state->next_token.type == pLBRACE) {
    ...
    rbs_location_t *loc = alloc_location(&state->allocator, function_range);
...
static bool parse_proc_type(parserstate *state, rbs_types_proc_t **proc) {
  ...
  rbs_location_t *loc = alloc_location(&state->allocator, (range) { .start = start, .end = end });
...
static bool parse_record_attributes(parserstate *state, rbs_hash_t **fields) {
  *fields = rbs_hash_new(&state->allocator);
  ...
  while (true) {
    ...
    rbs_location_t *loc = alloc_location(&state->allocator, field_range);
...
static bool parse_instance_type(parserstate *state, bool parse_alias, rbs_node_t **type) {
    ...
    rbs_location_t *loc = alloc_location(&state->allocator, type_range);
...
static bool parse_simple(parserstate *state, rbs_node_t **type) {
  ...
  case pLBRACKET: {
    ...
    rbs_location_t *loc = alloc_location(&state->allocator, rg);
  ...
  *params = rbs_node_list_new(&state->allocator);
  ...
  if (state->next_token.type == pLBRACKET) {
    ...
    while (true) {
      ...
      rbs_keyword_t *variance = rbs_keyword_new(&state->allocator, alloc_location(&state->allocator, state->current_token.range), INTERN("invariant"));
      ...
      if (module_type_params) {
        ...
        if (state->next_token.type == kIN || state->next_token.type == kOUT) {
          switch (state->next_token.type) {
          case kIN:
            variance = rbs_keyword_new(&state->allocator, alloc_location(&state->allocator, state->current_token.range), INTERN("contravariant"));
            break;
          case kOUT:
            variance = rbs_keyword_new(&state->allocator, alloc_location(&state->allocator, state->current_token.range), INTERN("covariant"));
            break;
          default:
            set_error(state, state->current_token, false, "Unexpected error");
            return false;
      ...
      rbs_string_t string = rbs_parser_peek_current_token(state);
      rbs_location_t *nameSymbolLoc = alloc_location(&state->allocator, state->current_token.range);
...
static bool parse_global_decl(parserstate *state, rbs_ast_declarations_global_t **global) {
  ...
  range name_range = state->current_token.range;
  rbs_location_t *symbolLoc = alloc_location(&state->allocator, name_range);
  ...
  rbs_location_t *loc = alloc_location(&state->allocator, decl_range);
  rbs_loc_alloc_children(loc, 2);
  rbs_loc_add_required_child(loc, INTERN("name"), name_range);
...
static bool parse_annotation(parserstate *state, rbs_ast_annotation_t **annotation) {
  ...
  *annotation = rbs_ast_annotation_new(&state->allocator, alloc_location(&state->allocator, rg), stripped_annotation_str);
  return true;
}
...
static bool parse_method_name(parserstate *state, range *range, rbs_ast_symbol_t **symbol) {
  ...
  KEYWORD_CASES
    if (state->next_token.type == pQUESTION && state->current_token.range.end.byte_pos == state->next_token.range.start.byte_pos) {
      ...
      rbs_location_t *symbolLoc = alloc_location(&state->allocator, *range);
...
static bool parse_member_def(parserstate *state, bool instance_only, bool accept_overload, position comment_pos, rbs_node_list_t *annotations, rbs_ast_members_methoddefinition_t **method_definition) {
  ...
  case kPRIVATE: {
    ...
    visibility = rbs_keyword_new(&state->allocator, alloc_location(&state->allocator, visibility_range), INTERN("private"));
  ...
  case kPUBLIC: {
    visibility_range = state->current_token.range;
    visibility = rbs_keyword_new(&state->allocator, alloc_location(&state->allocator, visibility_range), INTERN("public"));
    member_range.start = visibility_range.start;
    parser_advance(state);
    break;
  }
    ...
    case pQUESTION:
      {
        ...
        rbs_location_t *loc = alloc_location(&state->allocator, overload_range);
  ...
  switch (kind) {
  case INSTANCE_KIND: {
    k = rbs_keyword_new(&state->allocator, alloc_location(&state->allocator, state->current_token.range), INTERN("instance"));
    break;
  }
  case SINGLETON_KIND: {
    k = rbs_keyword_new(&state->allocator, alloc_location(&state->allocator, state->current_token.range), INTERN("singleton"));
    break;
  }
  case INSTANCE_SINGLETON_KIND: {
    k = rbs_keyword_new(&state->allocator, alloc_location(&state->allocator, state->current_token.range), INTERN("singleton_instance"));
    break;
  }
  default:
    set_error(state, state->current_token, false, "Unexpected error");
  ...
  rbs_location_t *loc = alloc_location(&state->allocator, member_range);
  rbs_loc_alloc_children(loc, 5);
  rbs_loc_add_required_child(loc, INTERN("keyword"), keyword_range);
  rbs_loc_add_required_child(loc, INTERN("name"), name_range);
...
static bool parse_alias_member(parserstate *state, bool instance_only, position comment_pos, rbs_node_list_t *annotations, rbs_ast_members_alias_t **alias_member) {
  ...
  if (!instance_only && state->next_token.type == kSELF) {
    kind = rbs_keyword_new(&state->allocator, alloc_location(&state->allocator, state->current_token.range), INTERN("singleton"));
    ...
    kind = rbs_keyword_new(&state->allocator, alloc_location(&state->allocator, state->current_token.range), INTERN("instance"));
    CHECK_PARSE(parse_method_name(state, &new_name_range, &new_name));
    CHECK_PARSE(parse_method_name(state, &old_name_range, &old_name));
...
static bool parse_visibility_member(parserstate *state, rbs_node_list_t *annotations, rbs_node_t **visibility_member) {
  ...
  rbs_location_t *location = alloc_location(&state->allocator, state->current_token.range);
...
static bool parse_attribute_member(parserstate *state, position comment_pos, rbs_node_list_t *annotations, rbs_node_t **attribute_member) {
  ...
  rbs_keyword_t *kind = rbs_keyword_new(
    &state->allocator,
    alloc_location(&state->allocator, keyword_range),
  ...
  if (state->next_token.type == pLPAREN) {
    ...
    if (parser_advance_if(state, tAIDENT)) {
      ...
      ivar_name = (rbs_node_t *) rbs_ast_bool_new(&state->allocator, alloc_location(&state->allocator, false_range), false);
...
static bool parse_module_self_types(parserstate *state, rbs_node_list_t *array) {
  while (true) {
    ...
    rbs_location_t *loc = alloc_location(&state->allocator, self_range);
    rbs_loc_alloc_children(loc, 2);
    rbs_loc_add_required_child(loc, INTERN("name"), name_range);
    rbs_loc_add_optional_child(loc, INTERN("args"), args_range);
...
static bool parse_class_decl_super(parserstate *state, range *lt_range, rbs_ast_declarations_class_super_t **super) {
  if (parser_advance_if(state, pLT)) {
    *lt_range = state->current_token.range;
    ...
    rbs_location_t *loc = alloc_location(&state->allocator, super_range);
    rbs_loc_alloc_children(loc, 2);
    rbs_loc_add_required_child(loc, INTERN("name"), name_range);
    rbs_loc_add_optional_child(loc, INTERN("args"), args_range);
...
static bool parse_namespace(parserstate *state, range *rg, rbs_namespace_t **namespace) {
  ...
  *namespace = rbs_namespace_new(&state->allocator, alloc_location(&state->allocator, *rg), path, is_absolute);
  return true;
}
  ...
  while (true) {
    ...
    switch (state->next_token.type)
    {
      ...
      case tUIDENT: {
        ...
        rbs_typename_t *type_name = rbs_typename_new(&state->allocator, alloc_location(&state->allocator, type_name_range),namespace, symbol);
        ...
        if (state->next_token.type == kAS) {
          ...
          rbs_location_t *symbolLoc = alloc_location(&state->allocator, new_name_range);
        ...
        rbs_location_t *loc = alloc_location(&state->allocator, clause_range);
        rbs_loc_alloc_children(loc, 3);
...
static bool parse_use_directive(parserstate *state, rbs_ast_directives_use_t **use_directive) {
  if (state->next_token.type == kUSE) {
    ...
    rbs_location_t *loc = alloc_location(&state->allocator, directive_range);
    rbs_loc_alloc_children(loc, 1);
    rbs_loc_add_required_child(loc, INTERN("keyword"), keyword_range);
...
bool parse_signature(parserstate *state, rbs_signature_t **signature) {
  ...
  *signature = rbs_signature_new(&state->allocator, alloc_location(&state->allocator, signature_range), dirs, decls);
  return true;
}
