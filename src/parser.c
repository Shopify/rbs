...
static bool parse_type_name(parserstate *state, TypeNameKind kind, range *rg, rbs_typename_t **typename) {
  ...
  while (
    ...
    rbs_ast_symbol_t *symbol = rbs_ast_symbol_new(symbolLoc, &state->constant_pool, symbol_value);
  ...
  rbs_namespace_t *namespace = rbs_namespace_new(loc, path, absolute);
  ...
  success: {
    ...
    rbs_ast_symbol_t *symbol = rbs_ast_symbol_new(symbolLoc, &state->constant_pool, name);
    *typename = rbs_typename_new(rbs_location_new(*rg), namespace, symbol);
    return true;
  }
...
static bool parse_function_param(parserstate *state, rbs_types_function_param_t **function_param) {
  ...
  if (state->next_token.type == pCOMMA || state->next_token.type == pRPAREN) {
    ...
    *function_param = rbs_types_function_param_new(loc, type, NULL);
    return true;
  } else {
    ...
    rbs_ast_symbol_t *name = rbs_ast_symbol_new(symbolLoc, &state->constant_pool, constant_id);

    rbs_location_t *loc = rbs_location_new(param_range);
    rbs_loc_alloc_children(loc, 1);
    rbs_loc_add_optional_child(loc, INTERN("name"), name_range);

    *function_param = rbs_types_function_param_new(loc, type, name);
    return true;
  }
}
...
static bool parse_keyword_key(parserstate *state, rbs_ast_symbol_t **key) {
  ...
  if (state->next_token.type == pQUESTION) {
    ...
    *key = rbs_ast_symbol_new(symbolLoc, &state->constant_pool, INTERN_TOKEN(state, state->current_token));
  }
...
static bool parse_optional(parserstate *state, rbs_node_t **optional) {
  ...
  if (state->next_token.type == pQUESTION) {
    ...
    *optional = (rbs_node_t *) rbs_types_optional_new(location, type);
  } else {
    *optional = type;
  }
...
static bool parse_proc_type(parserstate *state, rbs_types_proc_t **proc) {
  ...
  *proc = rbs_types_proc_new(loc, result->function, result->block, result->function_self_type);
  return true;
}
...
static bool parse_symbol(parserstate *state, rbs_location_t *location, rbs_types_literal_t **symbol) {
  ...
  case tSYMBOL: {
    ...
    literal = rbs_ast_symbol_new(symbolLoc, &state->constant_pool, constant_id);
    break;
  }
  case tDQSYMBOL:
  case tSQSYMBOL: {
  ...
  *symbol = rbs_types_literal_new(location, (rbs_node_t *) literal);
  return true;
}
...
static bool parse_instance_type(parserstate *state, bool parse_alias, rbs_node_t **type) {
    ...
    if (kind == CLASS_NAME) {
      ...
      *type = (rbs_node_t *) rbs_types_alias_new(loc, typename, types);
    }
...
static bool parse_simple(parserstate *state, rbs_node_t **type) {
  ...
  case kUNTYPED: {
    rbs_location_t *loc = rbs_location_current_token(state);
    *type = (rbs_node_t *) rbs_types_bases_any_new(loc, false);
    return true;
  }
  case k__TODO__: {
    rbs_location_t *loc = rbs_location_current_token(state);
    *type = (rbs_node_t *) rbs_types_bases_any_new(loc, true);
    return true;
  }
  case tINTEGER: {
    rbs_location_t *loc = rbs_location_current_token(state);
  ...
  case pLBRACKET: {
    ...
    *type = (rbs_node_t *) rbs_types_tuple_new(loc, types);
    return true;
  }
  case pAREF_OPR: {
    rbs_location_t *loc = rbs_location_current_token(state);
  ...
  case pLBRACE: {
    ...
    if (!parse_record_attributes(state, &fields)) {
      rbs_hash_free(fields);
      return false;
    }
    ADVANCE_ASSERT(state, pRBRACE);
    position end = state->current_token.range.end;
    rbs_location_t *loc = rbs_location_pp(&start, &end);
    *type = (rbs_node_t *) rbs_types_record_new(loc, fields);
    return true;
  }
  case pHAT: {
...
static bool parse_type_params(parserstate *state, range *rg, bool module_type_params, rbs_node_list_t **params) {
  *params = rbs_node_list_new();
  ...
  if (state->next_token.type == pLBRACKET) {
    ...
    while (true) {
      rbs_keyword_t *variance = rbs_keyword_new(rbs_location_new(state->current_token.range), INTERN("invariant"));
      ...
      if (module_type_params) {
        ...
        if (state->next_token.type == kIN || state->next_token.type == kOUT) {
          switch (state->next_token.type) {
          case kIN:
            rbs_node_destroy((rbs_node_t *) variance);
            variance = rbs_keyword_new(rbs_location_new(state->current_token.range), INTERN("contravariant"));
            break;
          case kOUT:
            rbs_node_destroy((rbs_node_t *) variance);
            variance = rbs_keyword_new(rbs_location_new(state->current_token.range), INTERN("covariant"));
            break;
          default:
            rbs_node_destroy((rbs_node_t *) variance);
            set_error(state, state->current_token, false, "Unexpected error");
      ...
      rbs_ast_symbol_t *name = rbs_ast_symbol_new(nameSymbolLoc, &state->constant_pool, id);

      if (!parser_insert_typevar(state, id)) {
        rbs_node_destroy((rbs_node_t *) variance);
        rbs_node_destroy((rbs_node_t *) name);
        return false;
      }
      ...
      if (state->next_token.type == pLT) {
        ...
        if (!parse_type(state, &upper_bound)) {
          rbs_node_destroy((rbs_node_t *) variance);
          rbs_node_destroy((rbs_node_t *) name);
          return false;
        }
      ...
      if (module_type_params) {
        if (state->next_token.type == pEQ) {
          ...
          if (!parse_type(state, &default_type)) {
            rbs_node_destroy((rbs_node_t *) variance);
            rbs_node_destroy((rbs_node_t *) name);
            return false;
          }
...
bool parse_method_type(parserstate *state, rbs_methodtype_t **method_type) {
  ...
  if (!parse_type_params(state, &params_range, false, &type_params)) {
    rbs_node_list_free(type_params);
    return false;
  }
...
static bool parse_global_decl(parserstate *state, rbs_ast_declarations_global_t **global) {
  ...
  rbs_ast_symbol_t *typename = rbs_ast_symbol_new(symbolLoc, &state->constant_pool, INTERN_TOKEN(state, state->current_token));
  ...
  *global = rbs_ast_declarations_global_new(loc, typename, type, comment);
  return true;
}
...
static bool parse_const_decl(parserstate *state, rbs_ast_declarations_constant_t **constant) {
  ...
  *constant = rbs_ast_declarations_constant_new(loc, typename, type, comment);
  return true;
}
...
static bool parse_type_decl(parserstate *state, position comment_pos, rbs_node_list_t *annotations, rbs_ast_declarations_typealias_t **typealias) {
  ...
  range params_range;
  rbs_node_list_t *type_params;
  if (!parse_type_params(state, &params_range, true, &type_params)) {
    rbs_node_list_free(type_params);
    return false;
  }
...
static bool parse_annotation(parserstate *state, rbs_ast_annotation_t **annotation) {
  ...
  *annotation = rbs_ast_annotation_new(rbs_location_new(rg), stripped_annotation_str);
  return true;
}
...
static bool parse_method_name(parserstate *state, range *range, rbs_ast_symbol_t **symbol) {
  ...
  KEYWORD_CASES
    if (state->next_token.type == pQUESTION && state->current_token.range.end.byte_pos == state->next_token.range.start.byte_pos) {
      ...
      *symbol = rbs_ast_symbol_new(symbolLoc, &state->constant_pool, constant_id);
    } else {
      *range = state->current_token.range;
      rbs_location_t *symbolLoc = rbs_location_new(*range);
      *symbol = rbs_ast_symbol_new(symbolLoc, &state->constant_pool, INTERN_TOKEN(state, state->current_token));
    }
    return true;
...
static bool parse_member_def(parserstate *state, bool instance_only, bool accept_overload, position comment_pos, rbs_node_list_t *annotations, rbs_ast_members_methoddefinition_t **method_definition) {
  ...
  case kPRIVATE: {
    ...
    visibility = rbs_keyword_new(rbs_location_new(visibility_range), INTERN("private"));
  ...
  case kPUBLIC: {
    visibility_range = state->current_token.range;
    visibility = rbs_keyword_new(rbs_location_new(visibility_range), INTERN("public"));
    member_range.start = visibility_range.start;
    parser_advance(state);
    break;
  }
  ...
  if (!parse_method_name(state, &name_range, &name)) {
    if (visibility != NULL) rbs_node_destroy((rbs_node_t *) visibility);
    return false;
  }
  ...
  if (!parser_pop_typevar_table(state)) {
    if (visibility != NULL) rbs_node_destroy((rbs_node_t *) visibility);
    rbs_node_list_free(overloads);
    return false;
  }
  ...
  switch (kind) {
  case INSTANCE_KIND: {
    k = rbs_keyword_new(rbs_location_new(state->current_token.range), INTERN("instance"));
    break;
  }
  case SINGLETON_KIND: {
    k = rbs_keyword_new(rbs_location_new(state->current_token.range), INTERN("singleton"));
    break;
  }
  case INSTANCE_SINGLETON_KIND: {
    k = rbs_keyword_new(rbs_location_new(state->current_token.range), INTERN("singleton_instance"));
    break;
  }
  default:
    if (visibility != NULL) rbs_node_destroy((rbs_node_t *) visibility);
    rbs_node_list_free(overloads);
    rbs_node_destroy((rbs_node_t *) k);
    set_error(state, state->current_token, false, "Unexpected error");
    return false;
  }
...
static bool parse_mixin_member(parserstate *state, bool from_interface, position comment_pos, rbs_node_list_t *annotations, rbs_node_t **mixin_member) {
  ...
  {
  case kINCLUDE:
    *mixin_member = (rbs_node_t *) rbs_ast_members_include_new(loc, name, args, annotations, comment);
    return true;
  case kEXTEND:
    *mixin_member = (rbs_node_t *) rbs_ast_members_extend_new(loc, name, args, annotations, comment);
    return true;
  case kPREPEND:
    *mixin_member = (rbs_node_t *) rbs_ast_members_prepend_new(loc, name, args, annotations, comment);
    return true;
  default:
    set_error(state, state->current_token, false, "Unexpected error");
    return false;
...
static bool parse_alias_member(parserstate *state, bool instance_only, position comment_pos, rbs_node_list_t *annotations, rbs_ast_members_alias_t **alias_member) {
  ...
  if (!instance_only && state->next_token.type == kSELF) {
    kind = rbs_keyword_new(rbs_location_new(state->current_token.range), INTERN("singleton"));

    new_kind_range.start = state->next_token.range.start;
    new_kind_range.end = state->next_token2.range.end;
    ADVANCE_ASSERT(state, kSELF);
    ADVANCE_ASSERT(state, pDOT);
    if (!parse_method_name(state, &new_name_range, &new_name)) {
      rbs_node_destroy((rbs_node_t *) kind);
      return false;
    }
    ...
    ADVANCE_ASSERT(state, kSELF);
    ADVANCE_ASSERT(state, pDOT);
    if (!parse_method_name(state, &old_name_range, &old_name)) {
      rbs_node_destroy((rbs_node_t *) kind);
      return false;
    }
  } else {
    kind = rbs_keyword_new(rbs_location_new(state->current_token.range), INTERN("instance"));
    if (!parse_method_name(state, &new_name_range, &new_name)
      || !parse_method_name(state, &old_name_range, &old_name)) {
      rbs_node_destroy((rbs_node_t *) kind);
      return false;
    }
  ...
  *alias_member = rbs_ast_members_alias_new(loc, new_name, old_name, kind, annotations, comment);
  return true;
}
...
static bool parse_variable_member(parserstate *state, position comment_pos, rbs_node_list_t *annotations, rbs_node_t **variable_member) {
  ...
  case tAIDENT: {
    range name_range = state->current_token.range;
    rbs_location_t *symbolLoc = rbs_location_new(name_range);
    rbs_ast_symbol_t *name = rbs_ast_symbol_new(symbolLoc, &state->constant_pool, INTERN_TOKEN(state, state->current_token));
...
static bool parse_attribute_member(parserstate *state, position comment_pos, rbs_node_list_t *annotations, rbs_node_t **attribute_member) {
  ...
  if (!parse_method_name(state, &name_range, &attr_name)) {
    rbs_node_destroy((rbs_node_t *) kind);
    return false;
  }
  ...
  if (state->next_token.type == pLPAREN) {
    ...
    if (parser_advance_if(state, tAIDENT)) {
      rbs_location_t *symbolLoc = rbs_location_new(state->current_token.range);
      ivar_name = (rbs_node_t *) rbs_ast_symbol_new(symbolLoc, &state->constant_pool, INTERN_TOKEN(state, state->current_token));
      ...
      ivar_name = (rbs_node_t *) rbs_ast_bool_new(rbs_location_new(false_range), false);
...
static bool parse_interface_decl(parserstate *state, position comment_pos, rbs_node_list_t *annotations, rbs_ast_declarations_interface_t **interface_decl) {
  ...
  rbs_node_list_t *type_params;
  if (!parse_type_params(state, &type_params_range, true, &type_params)) {
    rbs_node_list_free(type_params);
    return false;
  }
  ...
  *interface_decl = rbs_ast_declarations_interface_new(loc, name, type_params, members, annotations, comment);
  return true;
}
...
static bool parse_module_self_types(parserstate *state, rbs_node_list_t *array) {
  while (true) {
    ...
    rbs_ast_declarations_module_self_t *self_type = rbs_ast_declarations_module_self_new(loc, module_name, args);
...
static bool parse_module_decl0(parserstate *state, range keyword_range, rbs_typename_t *module_name, range name_range, rbs_ast_comment_t *comment, rbs_node_list_t *annotations, rbs_ast_declarations_module_t **module_decl) {
  ...
  if (state->next_token.type == pCOLON) {
    ...
    if (!parse_module_self_types(state, self_types)) {
      rbs_node_list_free(self_types);
      return false;
    }
  ...
  *module_decl = rbs_ast_declarations_module_new(loc, module_name, type_params, self_types, members, annotations, comment);
  return true;
}
...
static bool parse_class_decl_super(parserstate *state, range *lt_range, rbs_ast_declarations_class_super_t **super) {
  if (parser_advance_if(state, pLT)) {
    *lt_range = state->current_token.range;
    ...
    if (!class_instance_name(state, CLASS_NAME, args, &name_range, &args_range, &name)) {
      rbs_node_list_free(args);
      return false;
    }
    ...
    *super = rbs_ast_declarations_class_super_new(loc, name, args);
  } else {
    *lt_range = NULL_RANGE;
  }
...
static bool parse_class_decl0(parserstate *state, range keyword_range, rbs_typename_t *name, range name_range, rbs_ast_comment_t *comment, rbs_node_list_t *annotations, rbs_ast_declarations_class_t **class_decl) {
  ...
  *class_decl = rbs_ast_declarations_class_new(loc, name, type_params, super, members, annotations, comment);
  return true;
}
...
static bool parse_decl(parserstate *state, rbs_node_t **decl) {
  ...
  if (!parse_annotations(state, annotations, &annot_pos)) {
    rbs_node_list_free(annotations);
    return false;
  }
  parser_advance(state);
  ...
  case pCOLON2: {
    rbs_ast_declarations_constant_t *constant = NULL;
    if (!parse_const_decl(state, &constant)) {
      rbs_node_list_free(annotations);
      return false;
    }
    *decl = (rbs_node_t *) constant;
  ...
  case tGIDENT: {
    rbs_ast_declarations_global_t *global = NULL;
    if (!parse_global_decl(state, &global)) {
      rbs_node_list_free(annotations);
      return false;
    }
    *decl = (rbs_node_t *) global;
  ...
  case kTYPE: {
    rbs_ast_declarations_typealias_t *typealias = NULL;
    if (!parse_type_decl(state, annot_pos, annotations, &typealias)) {
      rbs_node_list_free(annotations);
      return false;
    }
    *decl = (rbs_node_t *) typealias;
  ...
  case kINTERFACE: {
    rbs_ast_declarations_interface_t *interface_decl = NULL;
    if (!parse_interface_decl(state, annot_pos, annotations, &interface_decl)) {
      rbs_node_list_free(annotations);
      return false;
    }
    *decl = (rbs_node_t *) interface_decl;
  ...
  case kMODULE: {
    rbs_node_t *module_decl = NULL;
    if (!parse_module_decl(state, annot_pos, annotations, &module_decl)) {
      rbs_node_list_free(annotations);
      return false;
    }
    *decl = module_decl;
  ...
  case kCLASS: {
    rbs_node_t *class_decl = NULL;
    if (!parse_class_decl(state, annot_pos, annotations, &class_decl)) {
      rbs_node_list_free(annotations);
      return false;
    }
    *decl = class_decl;
...
static bool parse_namespace(parserstate *state, range *rg, rbs_namespace_t **namespace) {
  ...
  while (true) {
    if (state->next_token.type == tUIDENT && state->next_token2.type == pCOLON2) {
      rbs_location_t *symbolLoc = rbs_location_new(state->next_token.range);
      rbs_ast_symbol_t *symbol = rbs_ast_symbol_new(symbolLoc, &state->constant_pool, INTERN_TOKEN(state, state->next_token));
      rbs_node_list_append(path, (rbs_node_t *)symbol);
  ...
  *namespace = rbs_namespace_new(rbs_location_new(*rg), path, is_absolute);
  return true;
}
  ...
  while (true) {
    ...
    switch (state->next_token.type)
    {
      ...
      case tUIDENT: {
        ...
        rbs_location_t *symbolLoc = rbs_location_new(state->current_token.range);
        rbs_ast_symbol_t *symbol = rbs_ast_symbol_new(symbolLoc, &state->constant_pool, INTERN_TOKEN(state, state->current_token));
        rbs_typename_t *type_name = rbs_typename_new(rbs_location_new(type_name_range),namespace, symbol);
        ...
        if (state->next_token.type == kAS) {
          ...
          new_name = rbs_ast_symbol_new(symbolLoc, &state->constant_pool, INTERN_TOKEN(state, state->current_token));
...
static bool parse_use_directive(parserstate *state, rbs_ast_directives_use_t **use_directive) {
  if (state->next_token.type == kUSE) {
    ...
    if (!parse_use_clauses(state, clauses)) {
      rbs_node_list_free(clauses);
      return false;
    }
    ...
    *use_directive = rbs_ast_directives_use_new(loc, clauses);
  }
...
bool parse_signature(parserstate *state, rbs_signature_t **signature) {
  ...
  while (state->next_token.type == kUSE) {
    ...
    if (!parse_use_directive(state, &use_node)) {
      rbs_node_list_free(dirs);
      rbs_node_list_free(decls);
      return false;
    }
  ...
  *signature = rbs_signature_new(rbs_location_new(signature_range), dirs, decls);
  return true;
}
