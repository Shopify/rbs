...
/*
  type_name ::= {`::`} (tUIDENT `::`)* <tXIDENT>
              ...
              | {<tXIDENT>}
*/
static rbs_type_name_t *parse_type_name(parserstate *state, TypeNameKind kind, range *rg) {
  ...
  success: {
    ...
    return rbs_type_name_new(rbs_struct_to_ruby_value((rbs_node_t *)namespace), ID2SYM(INTERN_TOKEN(state, state->current_token)));
  }
...
static void parse_keyword(parserstate *state, VALUE keywords, VALUE memo) {
  ...
  rb_hash_aset(keywords, key, rbs_struct_to_ruby_value((rbs_node_t *)param));
  ...
  while (true) {
    switch (state->next_token.type) {
      ...
      default:
        ...
        rb_ary_push(params->required_positionals, rbs_struct_to_ruby_value((rbs_node_t *)param));
  ...
  while (true) {
    switch (state->next_token.type) {
      case pQUESTION:
        ...
        rb_ary_push(params->optional_positionals, rbs_struct_to_ruby_value((rbs_node_t *)param));
...
PARSE_REST_PARAM:
  if (state->next_token.type == pSTAR) {
    parser_advance(state);
    params->rest_positionals = rbs_struct_to_ruby_value((rbs_node_t *)parse_function_param(state));
  ...
  while (true) {
    switch (state->next_token.type) {
      ...
      default:
        ...
        rb_ary_push(params->trailing_positionals, rbs_struct_to_ruby_value((rbs_node_t *)param));
...
PARSE_KEYWORDS:
  while (true) {
    ...
    case pSTAR2:
      parser_advance(state);
      params->rest_keywords = rbs_struct_to_ruby_value((rbs_node_t *)parse_function_param(state));
      break;
...
static rbs_node_t *parse_optional(parserstate *state) {
  ...
  if (state->next_token.type == pQUESTION) {
    ...
    VALUE value_type = rbs_struct_to_ruby_value(type);
    return (rbs_node_t *)rbs_types_optional_new(value_type, location);
  } else {
    return type;
  }
}
...
static VALUE parse_record_attributes(parserstate *state) {
  ...
  while (true) {
    ...
    if (is_keyword(state)) {
      ...
      case kTRUE:
      case kFALSE: {
        rbs_node_t *simple_node = parse_simple(state);
        key = rb_funcall(rbs_struct_to_ruby_value(simple_node), rb_intern("literal"), 0);
        break;
      }
      default:
        raise_syntax_error(
...
static VALUE parse_instance_type(parserstate *state, bool parse_alias) {
    ...
    range name_range;
    rbs_type_name_t *typename = parse_type_name(state, expected_kind, &name_range);
...
static rbs_types_class_singleton_t *parse_singleton_type(parserstate *state) {
  ...
  range name_range;
  rbs_type_name_t *typename = parse_type_name(state, CLASS_NAME, &name_range);
...
static rbs_node_t *parse_simple(parserstate *state) {
  ...
  case kUNTYPED: {
    VALUE loc = rbs_location_current_token(state);
    return (rbs_node_t *)rbs_types_bases_any_new(Qfalse, loc);
  }
  case k__TODO__: {
    VALUE loc = rbs_location_current_token(state);
    return (rbs_node_t *)rbs_types_bases_any_new(Qtrue, loc);
  }
  case tINTEGER: {
    VALUE loc = rbs_location_current_token(state);
...
static VALUE parse_intersection(parserstate *state) {
  ...
  rb_ary_push(intersection_types, rbs_struct_to_ruby_value(type));
  while (state->next_token.type == pAMP) {
    parser_advance(state);
    rb_ary_push(intersection_types, rbs_struct_to_ruby_value(parse_optional(state)));
  }
...
static rbs_ast_declarations_constant_t *parse_const_decl(parserstate *state) {
  ...
  return rbs_ast_declarations_constant_new(rbs_struct_to_ruby_value((rbs_node_t *)typename), type, location, comment);
}
...
static rbs_ast_declarations_type_alias_t *parse_type_decl(parserstate *state, position comment_pos, VALUE annotations) {
  ...
  range name_range;
  rbs_type_name_t *typename = parse_type_name(state, ALIAS_NAME, &name_range);
  ...
  return rbs_ast_declarations_type_alias_new(
    rbs_struct_to_ruby_value((rbs_node_t *)typename),
    type_params,
    type,
    annotations,
    location,
  ...
  *annot_pos = NullPosition;
  ...
  while (true) {
    if (state->next_token.type == tANNOTATION) {
      ...
      rb_ary_push(*annotations, rbs_struct_to_ruby_value((rbs_node_t *)parse_annotation(state)));
    } else {
      break;
    }
  }
...
static rbs_ast_members_method_definition_t *parse_member_def(parserstate *state, bool instance_only, bool accept_overload, position comment_pos, VALUE annotations) {
  ...
  while (loop) {
    ...
    case pQUESTION:
      {
        rbs_method_type_t *method_type = parse_method_type(state);
        VALUE method_type_value = rbs_struct_to_ruby_value((rbs_node_t *)method_type);
        rb_ary_push(overloads, rbs_struct_to_ruby_value((rbs_node_t *)rbs_ast_members_method_definition_overload_new(annotations, method_type_value)));
 ...
 * @param kind
 * */
void class_instance_name(parserstate *state, TypeNameKind kind, rbs_type_name_t **name, VALUE *args, range *name_range, range *args_range) {
  parser_advance(state);
...
static rbs_node_t *parse_mixin_member(parserstate *state, bool from_interface, position comment_pos, VALUE annotations) {
  ...
  {
  case kINCLUDE:
    return (rbs_node_t *)rbs_ast_members_include_new(rbs_struct_to_ruby_value((rbs_node_t *)name), args, annotations, location, comment);
  case kEXTEND:
    return (rbs_node_t *)rbs_ast_members_extend_new(rbs_struct_to_ruby_value((rbs_node_t *)name), args, annotations, location, comment);
  case kPREPEND:
    return (rbs_node_t *)rbs_ast_members_prepend_new(rbs_struct_to_ruby_value((rbs_node_t *)name), args, annotations, location, comment);
  default:
    rbs_abort();
  }
}
 ...
 * */
static rbs_ast_members_alias_t *parse_alias_member(parserstate *state, bool instance_only, position comment_pos, VALUE annotations) {
  range member_range;
  member_range.start = state->current_token.range.start;
  range keyword_range = state->current_token.range;
  ...
  return rbs_ast_members_alias_new(
    new_name,
    old_name,
    kind,
    annotations,
...
static VALUE parse_interface_members(parserstate *state) {
  ...
  while (state->next_token.type != kEND) {
    ...
    case kALIAS: {
      member = rbs_struct_to_ruby_value((rbs_node_t *)parse_alias_member(state, true, annot_pos, annotations));
      break;
    }
...
static rbs_ast_declarations_interface_t *parse_interface_decl(parserstate *state, position comment_pos, VALUE annotations) {
  ...
  range name_range;
  rbs_type_name_t *name = parse_type_name(state, INTERFACE_NAME, &name_range);
  ...
  return rbs_ast_declarations_interface_new(
    rbs_struct_to_ruby_value((rbs_node_t *)name),
    params,
    members,
    annotations,
    location,
...
static void parse_module_self_types(parserstate *state, VALUE *array) {
  while (true) {
    ...
    range name_range;
    rbs_type_name_t *module_name = parse_type_name(state, CLASS_NAME | INTERFACE_NAME, &name_range);
    ...
    rbs_ast_declarations_module_self_t *self_type = rbs_ast_declarations_module_self_new(rbs_struct_to_ruby_value((rbs_node_t *)module_name), args, location);
    melt_array(array);
    rb_ary_push(*array, rbs_struct_to_ruby_value((rbs_node_t *)self_type));
...
static VALUE parse_module_members(parserstate *state) {
  ...
  while (state->next_token.type != kEND) {
    ...
    case kALIAS: {
      member = rbs_struct_to_ruby_value((rbs_node_t *)parse_alias_member(state, false, annot_pos, annotations));
      break;
    }
    ...
    case kPRIVATE:
      if (state->next_token.range.start.line == state->current_token.range.start.line) {
        ...
        member = rbs_struct_to_ruby_value((rbs_node_t *)parse_visibility_member(state, annotations));
      }
      break;
...
static rbs_ast_declarations_module_t *parse_module_decl0(parserstate *state, range keyword_range, VALUE module_name, range name_range, VALUE comment, VALUE annotations) {
  ...
  return rbs_ast_declarations_module_new(
    module_name,
    type_params,
    self_types,
    members,
...
static rbs_node_t *parse_module_decl(parserstate *state, position comment_pos, VALUE annotations) {
  ...
  rbs_type_name_t *module_name = parse_type_name(state, CLASS_NAME, &module_name_range);
  ...
  if (state->next_token.type == pEQ) {
    ...
    rbs_type_name_t *old_name = parse_type_name(state, CLASS_NAME, &old_name_range);
    ...
    return (rbs_node_t *)rbs_ast_declarations_module_alias_new(rbs_struct_to_ruby_value((rbs_node_t *)module_name), rbs_struct_to_ruby_value((rbs_node_t *)old_name), location, comment);
  } else {
    return (rbs_node_t *)parse_module_decl0(state, keyword_range, rbs_struct_to_ruby_value((rbs_node_t *)module_name), module_name_range, comment, annotations);
  }
}
...
static rbs_ast_declarations_class_super_t *parse_class_decl_super(parserstate *state, range *lt_range) {
  if (parser_advance_if(state, pLT)) {
    *lt_range = state->current_token.range;
    ...
    return rbs_ast_declarations_class_super_new(rbs_struct_to_ruby_value((rbs_node_t *)name), args, location);
  } else {
    *lt_range = NULL_RANGE;
    return NULL;
  }
...
static rbs_node_t *parse_class_decl(parserstate *state, position comment_pos, VALUE annotations) {
  ...
  rbs_type_name_t *class_name = parse_type_name(state, CLASS_NAME, &class_name_range);
  ...
  if (state->next_token.type == pEQ) {
    ...
    return (rbs_node_t *)rbs_ast_declarations_class_alias_new(rbs_struct_to_ruby_value((rbs_node_t *)class_name), rbs_struct_to_ruby_value((rbs_node_t *)old_name), location, comment);
  } else {
    return (rbs_node_t *)parse_class_decl0(state, keyword_range, rbs_struct_to_ruby_value((rbs_node_t *)class_name), class_name_range, comment, annotations);
  }
}
  ...
  while (true) {
    ...
    switch (state->next_token.type)
    {
      ...
      case tUIDENT: {
        ...
        rbs_type_name_t *type_name = rbs_type_name_new(rbs_struct_to_ruby_value((rbs_node_t *)namespace), ID2SYM(INTERN_TOKEN(state, state->current_token)));
        ...
        rb_ary_push(clauses, rbs_struct_to_ruby_value((rbs_node_t *)rbs_ast_directives_use_single_clause_new(rbs_struct_to_ruby_value((rbs_node_t *)type_name), new_name, location)));
...
VALUE parse_signature(parserstate *state) {
  ...
  while (state->next_token.type == kUSE) {
    melt_array(&dirs);
    rb_ary_push(dirs, rbs_struct_to_ruby_value((rbs_node_t *)parse_use_directive(state)));
  }
...
parse_method_type_try(VALUE a) {
  ...
  rbs_method_type_t *method_type = parse_method_type(arg->parser);
