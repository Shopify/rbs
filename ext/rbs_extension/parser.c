...
static rbs_typename_t *parse_type_name(parserstate *state, TypeNameKind kind, range *rg) {
  ...
  while (
    ...
    rbs_ast_symbol_t *symbol = rbs_ast_symbol_new(&state->allocator, symbol_value, symbol_value);
...
static rbs_types_function_param_t *parse_function_param(parserstate *state) {
  ...
  if (state->next_token.type == pCOMMA || state->next_token.type == pRPAREN) {
    ...
    rbs_ast_symbol_t *name = rbs_ast_symbol_new(&state->allocator, name_value, name_value);
    rbs_location_t *loc = rbs_location_new(state->buffer, param_range);
    rbs_loc_alloc_children(loc, 1);
...
static rbs_ast_symbol_t *parse_keyword_key(parserstate *state) {
  ...
  if (state->next_token.type == pQUESTION) {
    ...
    rbs_ast_symbol_t *key = rbs_ast_symbol_new(&state->allocator, key_value, key_value);
    parser_advance(state);
    return key;
  } else {
    VALUE key_value = ID2SYM(INTERN_TOKEN(state, state->current_token));
    return rbs_ast_symbol_new(&state->allocator, key_value, key_value);
  }
}
  ...
  while (true) {
    ...
    if (is_keyword(state)) {
      ...
      case kFALSE: {
        ...
        key = rbs_ast_symbol_new(&state->allocator, key_value, key_value);
        break;
      }
      default:
        raise_syntax_error(
    ...
    rbs_hash_set(fields, (rbs_node_t *) key, (rbs_node_t *) rbs_types_record_fieldtype_new(&state->allocator, value->cached_ruby_value, type, required));
...
static rbs_types_literal_t *parse_symbol(parserstate *state, rbs_location_t *location) {
  ...
  case tSYMBOL: {
    ...
    literal = rbs_ast_symbol_new(&state->allocator, literal_value, literal_value);
    break;
  }
  case tDQSYMBOL:
  case tSQSYMBOL: {
...
static rbs_node_list_t *parse_type_params(parserstate *state, range *rg, bool module_type_params) {
  ...
  if (state->next_token.type == pLBRACKET) {
    ...
    while (true) {
      ...
      rbs_ast_symbol_t *variance = rbs_ast_symbol_new(&state->allocator, variance_value, variance_value);
      ...
      if (module_type_params) {
        ...
        if (state->next_token.type == kIN || state->next_token.type == kOUT) {
          ...
          case kIN: {
            ...
            variance = rbs_ast_symbol_new(&state->allocator, variance_value, variance_value);
            break;
          }
          case kOUT: {
      ...
      rbs_ast_symbol_t *name = rbs_ast_symbol_new(&state->allocator, name_sym, name_sym);
...
static rbs_ast_declarations_global_t *parse_global_decl(parserstate *state) {
  ...
  rbs_ast_symbol_t *typename = rbs_ast_symbol_new(&state->allocator, typename_value, typename_value);
...
static rbs_ast_symbol_t *parse_method_name(parserstate *state, range *range) {
  ...
  KEYWORD_CASES
    if (state->next_token.type == pQUESTION && state->current_token.range.end.byte_pos == state->next_token.range.start.byte_pos) {
      ...
      return rbs_ast_symbol_new(&state->allocator, id_value, id_value);
    } else {
      *range = state->current_token.range;
      VALUE name_value = ID2SYM(INTERN_TOKEN(state, state->current_token));
      return rbs_ast_symbol_new(&state->allocator, name_value, name_value);
    }
  ...
  case tEQIDENT: {
    *range = state->current_token.range;
    VALUE symbol_value = ID2SYM(INTERN_TOKEN(state, state->current_token));
    return rbs_ast_symbol_new(&state->allocator, symbol_value, symbol_value);
  }
  case tQIDENT: {
...
static rbs_ast_members_methoddefinition_t *parse_member_def(parserstate *state, bool instance_only, bool accept_overload, position comment_pos, rbs_node_list_t *annotations) {
  ...
  case kPRIVATE: {
    ...
    visibility = rbs_ast_symbol_new(&state->allocator, visibility_value, visibility_value);
  ...
  case INSTANCE_KIND: {
    ...
    k = rbs_ast_symbol_new(&state->allocator, kind_value, kind_value);
    break;
  }
  case SINGLETON_KIND: {
...
static rbs_ast_members_alias_t *parse_alias_member(parserstate *state, bool instance_only, position comment_pos, rbs_node_list_t *annotations) {
  ...
  if (!instance_only && state->next_token.type == kSELF) {
    VALUE kind_value = ID2SYM(rb_intern("singleton"));
    kind = rbs_ast_symbol_new(&state->allocator, kind_value, kind_value);
...
static rbs_node_t *parse_attribute_member(parserstate *state, position comment_pos, rbs_node_list_t *annotations) {
  ...
  VALUE kind_value = ID2SYM(rb_intern((is_kind == INSTANCE_KIND) ? "instance" : "singleton"));
  rbs_ast_symbol_t *kind = rbs_ast_symbol_new(&state->allocator, kind_value, kind_value);
  ...
  if (state->next_token.type == pLPAREN) {
    ...
    if (parser_advance_if(state, tAIDENT)) {
      ...
      ivar_name = (rbs_node_t *) rbs_ast_symbol_new(&state->allocator, ivar_name_value, ivar_name_value);
  ...
  while (true) {
    ...
    switch (state->next_token.type)
    {
      ...
      case tUIDENT: {
        ...
        rbs_ast_symbol_t *symbol = rbs_ast_symbol_new(&state->allocator, type_name_value, type_name_value);
        ...
        if (state->next_token.type == kAS) {
          ...
          new_name = rbs_ast_symbol_new(&state->allocator, new_name_value, new_name_value);
