...
static rbs_node_t *parse_instance_type(parserstate *state, bool parse_alias) {
    ...
    if (kind == CLASS_NAME) {
      return (rbs_node_t *) rbs_types_classinstance_new(&state->allocator, typename, types->cached_ruby_value, location);
    } else if (kind == INTERFACE_NAME) {
      return (rbs_node_t *) rbs_types_interface_new(&state->allocator, typename, types->cached_ruby_value, location);
    } else if (kind == ALIAS_NAME) {
      return (rbs_node_t *) rbs_types_alias_new(&state->allocator, typename, types->cached_ruby_value, location);
    } else {
      return NULL;
    }
}
...
static rbs_node_t *parse_simple(parserstate *state) {
  ...
  case pLBRACKET: {
    ...
    VALUE location = rbs_new_location(state->buffer, rg);
    return (rbs_node_t *) rbs_types_tuple_new(&state->allocator, types->cached_ruby_value, location);
  }
  case pAREF_OPR: {
    VALUE loc = rbs_location_current_token(state);
    rbs_node_list_t *types = rbs_node_list_new();
...
static rbs_node_t *parse_mixin_member(parserstate *state, bool from_interface, position comment_pos, rbs_node_list_t *annotations) {
  ...
  {
  case kINCLUDE:
    return (rbs_node_t *) rbs_ast_members_include_new(&state->allocator, name, args->cached_ruby_value, annotations, location, comment);
  case kEXTEND:
    return (rbs_node_t *) rbs_ast_members_extend_new(&state->allocator, name, args->cached_ruby_value, annotations, location, comment);
  case kPREPEND:
    return (rbs_node_t *) rbs_ast_members_prepend_new(&state->allocator, name, args->cached_ruby_value, annotations, location, comment);
  default:
    rbs_abort();
  }
}
...
static rbs_ast_members_alias_t *parse_alias_member(parserstate *state, bool instance_only, position comment_pos, rbs_node_list_t *annotations) {
  ...
  return rbs_ast_members_alias_new(&state->allocator, ((rbs_node_t *)new_name)->cached_ruby_value, ((rbs_node_t *)old_name)->cached_ruby_value, ((rbs_node_t *)kind)->cached_ruby_value, annotations, location, comment);
}
...
static rbs_node_t *parse_variable_member(parserstate *state, position comment_pos, rbs_node_list_t *annotations) {
  ...
  case tAIDENT: {
    ...
    return (rbs_node_t *)rbs_ast_members_instancevariable_new(&state->allocator, ((rbs_node_t *)name)->cached_ruby_value, type, location, comment);
  }
  case tA2IDENT: {
    range name_range = state->current_token.range;
    rbs_ast_symbol_t *name = rbs_ast_symbol_new(&state->allocator, ID2SYM(INTERN_TOKEN(state, state->current_token)));
    ...
    return (rbs_node_t *) rbs_ast_members_classvariable_new(&state->allocator, ((rbs_node_t *)name)->cached_ruby_value, type, location, comment);
  }
  case kSELF: {
    range kind_range = {
    ...
    return (rbs_node_t *)rbs_ast_members_classinstancevariable_new(&state->allocator, ((rbs_node_t *)name)->cached_ruby_value, type, location, comment);
  }
  default:
    rbs_abort();
  }
...
static rbs_node_t *parse_attribute_member(parserstate *state, position comment_pos, rbs_node_list_t *annotations) {
  ...
  {
  case kATTRREADER:
    return (rbs_node_t *) rbs_ast_members_attrreader_new(&state->allocator, ((rbs_node_t *)attr_name)->cached_ruby_value, type, ivar_name, kind, annotations, location, comment, visibility_value);
  case kATTRWRITER:
    return (rbs_node_t *) rbs_ast_members_attrwriter_new(&state->allocator, ((rbs_node_t *)attr_name)->cached_ruby_value, type, ivar_name, kind, annotations, location, comment, visibility_value);
  case kATTRACCESSOR:
    return (rbs_node_t *) rbs_ast_members_attraccessor_new(&state->allocator, ((rbs_node_t *)attr_name)->cached_ruby_value, type, ivar_name, kind, annotations, location, comment, visibility_value);
  default:
    rbs_abort();
  }
}
...
static void parse_module_self_types(parserstate *state, rbs_node_list_t *array) {
  while (true) {
    ...
    rbs_ast_declarations_module_self_t *self_type = rbs_ast_declarations_module_self_new(&state->allocator, value, ((rbs_node_t *)module_name)->cached_ruby_value, args->cached_ruby_value, location);
...
*/
static rbs_ast_declarations_module_t *parse_module_decl0(parserstate *state, range keyword_range, VALUE module_name, range name_range, VALUE comment, rbs_node_list_t *annotations) {
  parser_push_typevar_table(state, true);
  ...
  return rbs_ast_declarations_module_new(&state->allocator, module_name, type_params, self_types->cached_ruby_value, members, annotations, location, comment);
}
...
static rbs_ast_declarations_class_super_t *parse_class_decl_super(parserstate *state, range *lt_range) {
  if (parser_advance_if(state, pLT)) {
    *lt_range = state->current_token.range;
    ...
    return rbs_ast_declarations_class_super_new(&state->allocator, name, args->cached_ruby_value, location);
  } else {
    *lt_range = NULL_RANGE;
    return NULL;
  }
...
*/
static rbs_ast_declarations_class_t *parse_class_decl0(parserstate *state, range keyword_range, VALUE name, range name_range, VALUE comment, rbs_node_list_t *annotations) {
  parser_push_typevar_table(state, true);
