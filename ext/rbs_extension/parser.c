...
static rbs_types_proc_t *parse_proc_type(parserstate *state) {
  ...
  return rbs_types_proc_new(&state->allocator, function, block, loc, function_self_type);
}
...
rbs_methodtype_t *parse_method_type(parserstate *state) {
  ...
  return rbs_methodtype_new(&state->allocator, type_params, function, block, loc);
}
